...
The locked-output figure work currently provides:

- the reading-reliability panel used for Figure 2 development;
- the final SMN4Lang MEG reliability-boundary panel;
- a first complete **Figure 3** composite generated by `scripts/paper/build_figure3_smn4lang.py`, using only the locked SMN4Lang fMRI reliability and transfer summaries/participant tables;
- the AHBA molecular-null panel for Extended Data consideration;
- normalized source tables and source manifests from the versioned builders.

Figure 3 contains the prospective design, model-blind fMRI reliability, frozen causal E5-to-fMRI mapping, paired λ=0 versus λ=0.10 participant RSA display, and participant-level transfer-delta panel. It does not recompute neural or model outcomes.

The remaining main-manuscript artwork task is to complete **Figure 1** and **Figure 4a/c/d**, then integrate the final composites into the submission Word document. This is figure assembly, not new scientific inference.
