...
def dk_distribution(fig,d):
    ...
    ns.zero_line(ax);ax.set_xticks([0,1]);ax.set_xticklabels(['Left','Right']);ax.tick_params(axis='x',length=0);ax.set_ylabel('DK parcel mean ΔRSA (×10$^{-3}$)');vals=dk['delta_mean'].to_numpy(float)*1e3;pad=.12*np.ptp(vals);ax.set_ylim(min(0,vals.min())-pad,vals.max()+pad);ax.text(.98,.94,f"{int(np.sum(vals>0))}/68 parcel means > 0\nunthresholded characterization",transform=ax.transAxes,ha='right',va='top',fontsize=7,color=ns.ORANGE,linespacing=1.25);ns.panel_letter(fig,.505,.905,'b',13);ns.panel_title(fig,.535,.899,'Complete bilateral DK68 regional distribution',9.5)
def surfaces(fig,surf,norm,cmap):
    ...
    cax=fig.add_axes([.920,.175,.012,.145]);sm=ScalarMappable(norm=norm,cmap=cmap);sm.set_array([]);cb=fig.colorbar(sm,cax=cax);cb.outline.set_linewidth(.5);cb.ax.tick_params(labelsize=7);fig.text(.926,.337,'mean ΔRSA\n(×10$^{-3}$)',fontsize=7,ha='center');ns.panel_letter(fig,.018,.475,'c',13);ns.panel_title(fig,.048,.469,'Same complete unthresholded DK68 phenotype on fsaverage cortex',9.5)
def main()->int:
