...
def draw_traj(fig,d):
    ...
    for key in ['zuco','smn4lang_fmri']:
        ...
        offsets=[(-5,6),(6,-10),(0,6),(0,6),(0,6)] if key=='zuco' else [(-6,-11),(7,5),(0,6),(0,6),(0,6)]
        for (xx,yy,ll),(dx,dy) in zip(zip(x,y,LAB),offsets): ax.annotate(ll,(xx,yy),xytext=(dx,dy),textcoords='offset points',ha='center',fontsize=7.0,color=c)
    ax.set_xlabel('Semantic cost (−ΔSTS, ×10$^{-3}$)'); ax.set_ylabel('External ΔRSA (×10$^{-3}$)'); ns.zero_line(ax); ax.text(.03,.95,'ZuCo EEG',transform=ax.transAxes,ha='left',va='top',color=ns.BLUE,fontsize=8.5,fontweight='bold'); ax.text(.03,.86,'SMN4Lang fMRI',transform=ax.transAxes,ha='left',va='top',color=ns.ORANGE,fontsize=8.5,fontweight='bold')
