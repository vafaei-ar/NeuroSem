...
def draw_systems(fig,d):
    ...
    ymin=min(0,m.min());ymax=m.max();pad=.16*(ymax-ymin);ax.set_xlim(-.55,2.6);ax.set_ylim(ymin-pad,ymax+pad);ax.set_xticks(range(3));ax.set_xticklabels(names);ax.set_ylabel('Mean participant ΔRSA (×10$^{-3}$)');ax.scatter([],[],s=15,c=ns.ORANGE,label='Participant');ax.scatter([],[],s=22,c=ns.INK,label='Mean ± 1.96 SE');ax.plot([],[],color='#CFCFCF',lw=.8,label='Within-participant');ax.legend(loc='upper center',bbox_to_anchor=(.5,-.145),ncol=3,fontsize=6.4)
def draw_null(fig,d):
