...
def schematic(fig):
    ax=fig.add_axes([.035,.635,.435,.245]); ax.set_axis_off(); box(ax,0,.235,.30,.80,'ChineseEEG\nneural geometry\n(reproducible source)','#F0F0F0','#BDBDBD',7.2); arrow(ax,(.415,.55),(.245,.55)); ax.text(.33,.615,'train once',transform=ax.transAxes,ha='center',fontsize=7.6,fontweight='bold'); ax.text(.33,.43,'relational supervision\non pairwise geometry',transform=ax.transAxes,ha='center',va='top',fontsize=6.2,color='#555'); box(ax,.425,.70,.26,.84,'FROZEN\n(no further updates)',ns.BLUE,ns.BLUE,7.8,'white')
...
def reliability(fig,d):
    for i,key in enumerate(['zuco','fmri']):
        ...
        ax.set_ylabel('Cross-participant neural\nreliability (LOO)',fontsize=7.0)
        ns.facet_strip(ax,f"{q['label']} (n = {q['n']})",ns.BLUE_STRIP if key=='zuco' else ns.ORANGE_STRIP,8); ax.text(.97,.93,f"{q['n']}/{q['n']}\npositive",transform=ax.transAxes,ha='right',fontsize=8.2,color=q['color'],fontweight='bold'); ax.text(.97,.08,f"mean LOO = {m:.3f}",transform=ax.transAxes,ha='right',fontsize=6.6,color=q['color'])

def transfer(fig,d):
    for i,key in enumerate(['zuco','fmri']):
        q=d[key]; x0=.055+i*.470; bg='#EFF5FC' if key=='zuco' else '#FDF3EC'; fig.add_artist(FancyBboxPatch((x0-.018,.297),.440,.243,boxstyle='round,pad=.004,rounding_size=.012',facecolor=bg,edgecolor='none',transform=fig.transFigure,zorder=0)); delta=q['delta']*1e3; n=q['n']; a0=q['a0']; a1=q['a1']; ax=fig.add_axes([x0+.035,.345,.290,.150],zorder=2); ax.patch.set_alpha(0)
        for k,(v0,v1) in enumerate(zip(a0,a1),1): ax.plot([k,k],[v0,v1],color='#64748B',lw=.9,zorder=2)
        ax.scatter(range(1,n+1),a0,s=22,facecolors='white',edgecolors=q['color'],lw=1,zorder=4,label='Text-only'); ax.scatter(range(1,n+1),a1,s=24,c=q['color'],zorder=4,label='Neural-guided'); ymin=float(min(a0.min(),a1.min())); ymax=float(max(a0.max(),a1.max())); pad=max(.18*(ymax-ymin),1e-5); ax.set_xlim(.4,n+.6); ax.set_ylim(ymin-pad,ymax+pad); ax.set_xticks(range(1,n+1)); ax.tick_params(axis='x',labelsize=6.2); ax.set_xlabel('Participant',fontsize=7.4,labelpad=1); ax.set_ylabel('Residual model–brain RSA (r)',fontsize=7.0); ax.set_title(f"{q['label']} (n = {n})",color=q['color'],fontsize=9,fontweight='bold',pad=23); ax.legend(loc='lower center',bbox_to_anchor=(.5,1.01),ncol=2,fontsize=5.8,frameon=False,columnspacing=.8,handletextpad=.4)
        axi=fig.add_axes([x0+.352,.345,.062,.150],zorder=2); axi.patch.set_alpha(0); ns.zero_line(axi); ns.observations(axi,-.16,delta,q['color'],seed=i+7,width=.10,size=9); ns.half_violin(axi,.14,delta,ns.BLUE_FILL if key=='zuco' else ns.ORANGE_FILL,width=.34,bw=.5); mean=q['delta_mean']*1e3; ci=q['delta_ci']*1e3; ns.point_with_ci(axi,.14,mean,ci[0],ci[1],q['color'],ms=5.5); dmax=max(float(delta.max()),float(ci[1])); dmin=min(0,float(delta.min()),float(ci[0])); dpad=max(.12*(dmax-dmin),.04); axi.set_xlim(-.6,.9); axi.set_ylim(dmin-dpad,dmax+dpad); axi.set_xticks([]); axi.yaxis.tick_right(); axi.yaxis.set_label_position('right'); axi.tick_params(axis='y',labelsize=5.5,pad=1); axi.set_ylabel('ΔRSA\n(×10$^{-3}$)',fontsize=5.9,labelpad=1); axi.spines['bottom'].set_visible(False); axi.set_title('Δ (neural − text)',fontsize=7.4,pad=16); axi.text(.5,.05,f"mean = {mean:+.2f} ×10$^{{-3}}$",transform=axi.transAxes,ha='center',va='bottom',fontsize=5.8,fontweight='bold')
...
def runs(fig,d):
    ...
    for i,key in enumerate(['zuco','fmri']):
        ...
        ax.axvline(.5,color='#B0B0B0',lw=.8,ls=(0,(3,3))); ax.set_xlim(-.6,3.6); ax.set_ylim(-.15*max(vals),1.35*max(vals)); ax.set_xticks(xs); ax.set_xticklabels(['Primary','Seed\n20260829','Seed\n20260830','Seed\n20260831'],fontsize=6.1); ax.set_ylabel('Mean ΔRSA\n(×10$^{-3}$)',fontsize=6.6); ax.spines['bottom'].set_visible(False); fig.text(.115,y0+.036,f"{q['label']}\n(n = {q['n']})",ha='center',va='center',fontsize=7.4,color=q['color'],fontweight='bold')
...
def main()->int:
    ...
    d,_=load_data(); ns.use(); fig=plt.figure(figsize=(10.4,7.35),dpi=200); ns.fig_title(fig,1,'Frozen brain-derived supervision transfers across independent neural datasets',size=13.5); schematic(fig); reliability(fig,d); transfer(fig,d); runs(fig,d); ns.panel_letter(fig,.016,.930,'a',13); ns.panel_title(fig,.042,.925,'Study schematic',9.5); ns.panel_letter(fig,.545,.930,'b',13); ns.panel_title(fig,.571,.925,'Reliability gates in target datasets',9.5); ns.panel_letter(fig,.016,.590,'c',13); ns.panel_title(fig,.042,.585,'Participant-level residual RSA and paired transfer',9.5); ns.panel_letter(fig,.016,.300,'d',13); ns.panel_title(fig,.042,.295,'Optimization-run consistency',9.5)
