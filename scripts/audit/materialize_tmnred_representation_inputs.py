...
def main():
    ...
    for subject in subjects:
        for session in sessions:
            ...
            if set_rec.get("status") != "materialized":
                row["status"] = "set_unavailable"; infrastructure_failures.append(dict(row)); rows.append(row); continue
            try:
                meta = load_eeg_meta(root / set_rel); row["layout"] = meta.get("layout", "")
            except Exception as exc:
                row["status"] = f"metadata_error:{type(exc).__name__}:{exc}"; infrastructure_failures.append(dict(row)); rows.append(row); continue
            for key in ["nbchan", "trials", "pnts", "srate", "xmin", "xmax"]:
                row[key] = meta.get(key, "")
            try:
                companion_rel = data_companion(root, meta, set_rel)
            except Exception as exc:
                row["status"] = f"companion_resolution_error:{type(exc).__name__}:{exc}"; infrastructure_failures.append(dict(row)); rows.append(row); continue
            if companion_rel:
                comp_rec = annex_get(root, companion_rel); row["companion_path"] = companion_rel; row["companion_status"] = comp_rec.get("status")
            else:
                row["companion_status"] = "embedded_or_not_required"
            try:
                n_trials = int(meta.get("trials", -1)); nbchan = int(meta.get("nbchan", -1)); srate = float(meta.get("srate", -1)); xmin = float(meta.get("xmin", 999)); xmax = float(meta.get("xmax", -999))
            except Exception:
            ...
            structural_ok = nbchan == 30 and srate > 0 and n_trials > 0 and xmin <= -0.19 and xmax >= 1.99 and (not companion_rel or row["companion_status"] == "materialized")
            if not structural_ok:
                row["status"] = "structural_failure"; infrastructure_failures.append(dict(row))
            elif not row["passes_min_trials"]:
                row["status"] = "qc_excluded_below_min_retained_trials"; qc_exclusions.append(dict(row))
    ...
    for subject in subjects:
        ...
        (ready_subjects if len(sr) == 8 and all(r["status"] in ready_statuses for r in sr) else excluded_subjects).append(subject)

    common_item_failures = []
    common_items_by_session = {}
    ...
    payload = {
        ...
        "n_ready_sessions": sum(r["status"] in ready_statuses for r in rows), "n_qc_excluded_sessions": len(qc_exclusions),
    ...
    if common_item_failures:
        raise SystemExit(f"TMNRED common-item freeze had {len(common_item_failures)} failure(s)")
    print(json.dumps({"status": "ok", "n_ready_subjects": len(ready_subjects), "excluded_subjects": excluded_subjects,
                      "subjects_requiring_resampling_to_200hz": payload["subjects_requiring_resampling_to_200hz"],
                      "common_items": {k:v["n_common_items"] for k,v in common_items_by_session.items()}, "output_dir": str(outdir)}, indent=2))
