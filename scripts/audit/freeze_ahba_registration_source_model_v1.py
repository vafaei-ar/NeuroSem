...
def main() -> int:
    ...
    ap.add_argument(
        "--feasibility-summary",
        type=Path,
        default=Path("outputs/ahba_forward_model_feasibility_v1/latest/summary.json"),
    )
    ap.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/ahba_registration_source_model_freeze_v1/latest"),
    )
    args = ap.parse_args()
    ...
    for rel in spatial:
        ...
        if rel.lower().endswith("_electrodes.tsv"):
            ...
            eeg_like = [r for r in finite if str(r.get("name", "")).strip()]
            ...
            if len(eeg_like) == 128 and representative_rows is None:
    ...
    # Materialize the standard template resources, but do not calculate any EEG or
    # molecular quantity. These are public anatomical resources only.
    fsaverage_dir = None
    ...
    # Freeze only choices that do not depend on outcomes.
    frozen_conventions = {
    ...
    # Head-shape points are helpful but not made a hard requirement here because
    # a later geometric registration implementation can use all measured scalp
    # electrodes as model-blind surface constraints. We record their availability.
    ready_for_registration_implementation = len(blockers) == 0
    ...
    payload = {
        ...
        "ready_for_registration_implementation": ready_for_registration_implementation,
        "blockers": blockers,
    ...
    print(json.dumps({
        "status": "ready" if ready_for_registration_implementation else "blocked",
        "ready_for_registration_implementation": ready_for_registration_implementation,
        "blockers": blockers,
    ...
    return 0 if ready_for_registration_implementation else 2
