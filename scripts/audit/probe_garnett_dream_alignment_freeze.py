...
def source_family(rel: str) -> str:
    ...
    if parts[0] != "derivatives":
        return "raw"
    # Preserve enough hierarchy to distinguish author derivative families.
...
def event_structure(path: Path) -> dict[str, Any]:
    ...
    # Structural pair check only: ROWS then ROWE in event-table order.
...
def main() -> int:
    ...
    garnett_files: list[Path] = []
    for p in all_files:
        rel = str(p.relative_to(root))
        low = rel.lower()
        if "ses-garnettdream" in low:
            garnett_files.append(p)

    events = [p for p in garnett_files if p.name.endswith("_events.tsv") and is_materialized(p)]
    event_records: list[dict[str, Any]] = []
    by_key: defaultdict[tuple[str, int], list[dict[str, Any]]] = defaultdict(list)

    for p in events:
        rel = str(p.relative_to(root))
        sub = ent(SUB_RE, rel) or ""
        run_s = ent(RUN_RE, rel)
        if not sub or not run_s:
            continue
        rec = {
            "path": rel,
            "subject": sub,
        ...
        by_key[(sub, int(run_s))].append(rec)

    # Choose a structural reference event family deterministically: prefer the
    # author preproc/filtered_0.5_30 derivative when present because it matches
    # the established ChineseEEG analysis family; otherwise use the family with
    # the largest subject-run coverage. This is based only on file provenance.
    ...
    # Collapse duplicate paths per subject-run only within the selected family.
    # Expected chapter identity is the unique CHxx marker if available.
    selected_by_sr: dict[tuple[str, int], dict[str, Any]] = {}
    duplicate_selected: list[dict[str, Any]] = []
    for r in sorted(selected, key=lambda x: x["path"]):
        k = (r["subject"], r["run"])
        if k in selected_by_sr:
            duplicate_selected.append({"subject": k[0], "run": k[1], "paths": [selected_by_sr[k]["path"], r["path"]]})
        else:
            selected_by_sr[k] = r

    subject_runs: defaultdict[str, list[dict[str, Any]]] = defaultdict(list)
    valid_chapters_by_subject: defaultdict[str, list[int]] = defaultdict(list)
    exclusions: list[dict[str, Any]] = []

    for (sub, run), r in sorted(selected_by_sr.items()):
        chapters = r["chapter_markers"]
        chapter = chapters[0] if len(chapters) == 1 else None
        structurally_valid = bool(
            r["rows_rowe_strict_alternation"]
            and r["n_rows_start"] > 0
            and r["n_rows_start"] == r["n_rows_end"]
            and chapter is not None
        )
        reason = ""
        if not structurally_valid:
            reason = "invalid ROWS/ROWE pairing or non-unique/missing CHxx chapter marker"
        # CH19 is not a cross-subject replacement for CH18. It is a distinct
        # structural identity and must not be substituted post hoc.
        if chapter == 19:
            structurally_valid = False
            reason = "CH19 is unique replacement material for sub-07 and is not interchangeable with CH18"
        if structurally_valid:
            valid_chapters_by_subject[sub].append(int(chapter))
        else:
            exclusions.append({"subject": sub, "run": run, "chapter": chapter, "reason": reason, "path": r["path"]})
        subject_runs[sub].append({
            "run": run,
            "chapter": chapter,
            "n_items": r["n_structural_pairs"],
            "structurally_valid": structurally_valid,
            "reason": reason,
            "path": r["path"],
        })

    subjects = sorted(subject_runs)
    chapter_support = Counter()
    for sub in subjects:
        chapter_support.update(set(valid_chapters_by_subject[sub]))
    ...
    # Materialization inventory for selected EEG family. We record only paths,
    # file types, and availability; no signal sample is opened.
    materialization_rows: list[dict[str, Any]] = []
    for p in garnett_files:
        rel = str(p.relative_to(root))
        if selected_family and source_family(rel) != selected_family:
            continue
        if p.suffix.lower() not in EEG_EXTS and not p.name.endswith("_events.tsv") and not p.name.endswith("_channels.tsv") and not p.name.endswith("_eeg.json"):
            continue
        sub = ent(SUB_RE, rel) or ""
        run_s = ent(RUN_RE, rel)
        materialization_rows.append({
            ...
            "materialized": is_materialized(p),
            "is_signal_file": p.suffix.lower() in EEG_EXTS,
        })

    # Discover candidate public text/stimulus files without using neural outcomes.
    text_candidates: list[dict[str, Any]] = []
    for p in all_files:
        if p.suffix.lower() not in SMALL_TEXT_EXTS:
            continue
        rel = str(p.relative_to(root))
        low = rel.lower()
    ...
    summary = {
        ...
        "subjects": subjects,
        "n_selected_subject_runs": len(selected_by_sr),
        "duplicate_selected_subject_runs": duplicate_selected,
        "subject_runs": [{"subject": s, "runs": subject_runs[s]} for s in subjects],
        "chapter_support": {str(k): int(v) for k, v in sorted(chapter_support.items())},
        "chapters_supported_by_all_subjects": chapters_all,
        "chapters_supported_by_at_least_80pct_subjects": chapters_80,
        ...
        "freeze_gate": {
            "ready_for_materialization": bool(selected_family and subjects and not duplicate_selected),
            "ready_for_reliability": False,
            "reason": "Next freeze must identify the exact public text file/row mapping and materialize the selected EEG companions before outcome-bearing EEG reliability.",
        },
    }
    ...
    print(json.dumps({
        ...
        "chapters_80pct": chapters_80,
        "n_structural_exclusions": len(exclusions),
        "output_dir": str(out),
    }, indent=2))
    return 0
